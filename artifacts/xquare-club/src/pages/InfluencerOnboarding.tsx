...
const FOLLOWER_RANGES = ["50k–100k", "100k+"];
...
/* ── shared style tokens ── */
const inputClass =
  "w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-white/30 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-colors";
  ...
  return (
    <div className="grid sm:grid-cols-2 gap-2.5">
      {options.map((opt) => (
        ...
        >
          <div
            className={`w-4 h-4 rounded border-2 flex-shrink-0 flex items-center justify-center transition-colors ${selected.includes(opt) ? "bg-primary border-primary" : "border-white/30"}`}
          >
            {selected.includes(opt) && (
              <svg
                viewBox="0 0 10 8"
                className="w-2.5 h-2 text-white fill-none stroke-current"
              >
                <path
                  d="M1 4l3 3 5-6"
                  strokeWidth="1.5"
  ...
  return (
    <div className="flex flex-col gap-2.5">
      {options.map((opt) => (
        ...
        >
          <div
            className={`w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-colors ${selected === opt ? "border-primary" : "border-white/30"}`}
          >
            {selected === opt && (
...
export default function InfluencerOnboarding() {
  ...
  const validate = () => {
    ...
    if (!form.dashboardScreenshot)
      e.dashboardScreenshot = "Please upload your dashboard screenshot.";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      /* actual files */
      if (form.dashboardScreenshot)
        fd.append("dashboardScreenshot", form.dashboardScreenshot);
      ...
      <div className="min-h-screen bg-black text-white flex items-center justify-center px-6">
        ...
        >
          ...
          <p className="text-white/60 mb-8">
            Thank you for submitting. Our team will review your details and
            connect with you on the next steps.
          </p>
          <button
            onClick={() => (window.location.href = "/")}
            className="px-8 py-3 rounded-xl bg-primary text-white font-semibold hover:bg-blue-400 transition-colors"
          >
            Back to Home
          </button>
        </motion.div>
      ...
      <div className="max-w-3xl mx-auto px-6 py-12">
        ...
        >
          ...
          <p className="text-white/50 text-sm max-w-xl mx-auto">
            Submit your application to join XQUARE CLUB as an influencer.
          </p>
          <p className="text-white/50 text-sm max-w-xl mx-auto mt-2">
            Your details will be used for review, verification, onboarding, and
        ...
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          ...
          <SectionCard title="Consent">
            ...
            >
              <div
                className={`w-5 h-5 mt-0.5 rounded border-2 flex-shrink-0 flex items-center justify-center transition-colors ${form.consent ? "bg-primary border-primary" : "border-white/30"}`}
              >
                {form.consent && (
                  <svg
                    viewBox="0 0 10 8"
                    className="w-3 h-2.5 text-white fill-none stroke-current"
                  >
                    <path
                      d="M1 4l3 3 5-6"
                      strokeWidth="1.8"
              ...
              <span className="text-sm text-white/80 leading-relaxed">
                I am interested in Early Access and I agree that my submitted
                details may be used for onboarding and communication.{" "}
                <span className="text-red-400">*</span>
              </span>
            </button>
            {errors.consent && (
              <p className={errClass} data-err>
                {errors.consent}
              </p>
            )}
          </SectionCard>
          ...
          <SectionCard
            ...
            description="Please provide your main social media profile details."
          >
            <div>
              <label className={labelClass}>
                Primary social platform <span className="text-red-400">*</span>
              </label>
              <RadioGroup
                options={PLATFORMS}
            ...
            <div>
              <label className={labelClass}>
                Social profile link <span className="text-red-400">*</span>
              </label>
              <input
                className={inputClass}
                type="url"
            ...
            <div>
              <label className={labelClass}>
                Follower Count Range <span className="text-red-400">*</span>
              </label>
              <RadioGroup
                options={FOLLOWER_RANGES}
                selected={form.followerRange}
                onChange={(v) => set("followerRange", v)}
              />
          ...
          <SectionCard title="Niche">
            <div>
              <label className={labelClass}>
                Which niche best describes your content?{" "}
                <span className="text-red-400">*</span>
              </label>
          ...
          <SectionCard title="Brand Collaboration & Monetization">
            <div>
              <label className={labelClass}>
                How do you currently earn from your social media?
              </label>
              <CheckGroup
                options={EARN_OPTIONS}
                selected={form.earnMethods}
                onChange={(v) => set("earnMethods", v)}
              />
              {form.earnMethods.includes("Other") && (
                <input
                  className={inputClass + " mt-3"}
                  placeholder="Please specify..."
                  value={form.earnOther}
                  onChange={(e) => set("earnOther", e.target.value)}
                />
              )}
            </div>
            ...
            <div>
              <label className={labelClass}>
                How often do you get brand collaboration opportunities?{" "}
                <span className="text-red-400">*</span>
              </label>
              <RadioGroup
                options={COLLAB_FREQ}
                selected={form.collabFreq}
                onChange={(v) => set("collabFreq", v)}
              />
              {errors.collabFreq && (
                <p className={errClass} data-err>
                  {errors.collabFreq}
            ...
            <div>
              <label className={labelClass}>
                How do you usually get collaborations?
              </label>
              <CheckGroup
                options={COLLAB_SOURCE}
                selected={form.collabSource}
                onChange={(v) => set("collabSource", v)}
              />
              {form.collabSource.includes("Other") && (
                <input
                  className={inputClass + " mt-3"}
                  placeholder="Please specify..."
                  value={form.collabSourceOther}
                  onChange={(e) => set("collabSourceOther", e.target.value)}
                />
              )}
            </div>
            ...
            <div>
              <label className={labelClass}>
                How is your pricing usually decided?
              </label>
              <CheckGroup
                options={PRICING_METHOD}
                selected={form.pricingMethod}
                onChange={(v) => set("pricingMethod", v)}
              />
              {form.pricingMethod.includes("Other") && (
                <input
                  className={inputClass + " mt-3"}
                  placeholder="Please specify..."
                  value={form.pricingOther}
                  onChange={(e) => set("pricingOther", e.target.value)}
                />
              )}
            </div>
            ...
            <div>
              <label className={labelClass}>
                What is your typical paid collaboration price range?{" "}
                <span className="text-red-400">*</span>
              </label>
              <RadioGroup
          ...
          <SectionCard
            ...
            description="These details help us verify your profile for Early Access and onboarding. Your information will be kept confidential."
          >
            ...
            <div className="pt-2 border-t border-white/5 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  value={form.kycType}
                  onChange={(e) => set("kycType", e.target.value)}
                />
              </div>
              <div>
                <label className={labelClass}>
                ...
                <input
                  ...
                  onChange={(e) => set("kycNumber", e.target.value)}
                />
              </div>
              <FileInput
                label="KYC document upload / link"
                value={form.kycDoc}
                onChange={(f) => set("kycDoc", f)}
                accept="image/*,.pdf"
              />
            </div>
          </SectionCard>
          ...
          <p className="text-center text-white/25 text-xs pb-10">
            By submitting, you agree that XQUARE CLUB may use your information
            for onboarding and communication purposes.
          </p>
        </form>
      </div>
    </div>
